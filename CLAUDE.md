...
| `ai.js` | Analisis keuangan & chat berbasis AI. Mendukung 2 mesin (dipilih via `window.getAIEngine()`/`setAIEngine()`, disimpan di `sk_ai_engine`): `worker` (Cloudflare Worker + Groq, lama) dan `gemini` (Supabase Edge Function `supabase/functions/ai-gemini/`, fallback multi API key Gemini di sisi server). Semua fitur AI manggil lewat satu titik: `window.callAIEngine(prompt)` / `window.resolveAIEndpoint()` -- jangan `fetch` worker URL langsung lagi di fitur baru, pakai helper ini supaya otomatis ikut mesin yang dipilih user. Juga berisi **saran kategori otomatis**: saat Deskripsi transaksi (`txDesc`/`editTxDesc`) diisi, AI menyarankan kategori lewat chip "Saran AI: ... [Pakai]" (debounce 800ms, tidak pernah menimpa pilihan manual, diam kalau mesin AI belum dikonfigurasi) -- lihat `initAiCategorySuggest`. |
