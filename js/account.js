...
window.renderAccModalList = function(highlightId) {
    ...
    if (!accounts.length) { el.innerHTML = '<div style="text-align:center;color:#9C8B72;font-size:.75rem;padding:10px 0;">Belum ada akun tersimpan.</div>'; return; }
...
window.saveNewAccount = async function() {
    ...
    try {
        ...
        if (hasCredentials) {
            st.style.color='#9C8B72'; st.innerText=window.t('testing_supabase');
