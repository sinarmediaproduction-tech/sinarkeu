...
window.saveNewAccount = async function() {
    ...
    if (!name) { st.style.color='#A13A3A'; st.innerText=window.t('acc_name_required'); return; }
    const isEdit = !!editId;
    ...
    if (dupName) { st.style.color='#A13A3A'; st.innerText = `Nama akun "${name}" sudah dipakai. Gunakan nama lain.`; return; }

    if (!isEdit && (!url || !key || !pwd || pwd.length < 6)) { st.style.color='#A13A3A'; st.innerText='URL, Anon Key, dan Password (min 6 karakter) wajib diisi!'; return; }
    if (isEdit && hasCredentials && pwd.length < 6) { st.style.color='#A13A3A'; st.innerText='Password minimal 6 karakter!'; return; }
    if (hasCredentials && pwd !== pwd2) { st.style.color='#A13A3A'; st.innerText='Konfirmasi password tidak cocok! Pastikan kedua password sama.'; return; }
    ...
    if (hasCredentials) {
        st.style.color='#9AA2AC'; st.innerText = 'Memeriksa URL akun lain…';
    ...
    try {
        ...
        if (hasCredentials) {
            st.style.color='#9AA2AC'; st.innerText=window.t('testing_supabase');
