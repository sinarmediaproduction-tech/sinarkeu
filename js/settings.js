...
window.saveDeviceName = function() {
    ...
    if (raw) {
        ...
        if (!sanitized) {
            if (st) { st.style.color = '#8A2E28'; st.innerText = 'Nama tidak valid. Gunakan huruf, angka, spasi, atau tanda hubung.'; }
            return;
        }
        newId = sanitized;
    } else {
    ...
    if (st) { st.style.color = '#3E7A5D'; st.innerText = 'Tersimpan: ' + newId; }
...
window.loadConnectedDevices = async function() {
    ...
    try {
        ...
        if (!logs || !Array.isArray(logs) || logs.length === 0) {
            ...
            listEl.innerHTML = '<div style="font-size:.72rem; color:#A79C8B; text-align:center; padding:20px 0;">Belum ada log aktivitas di cloud.</div>';
            return;
        }
        ...
        devices.forEach(function(d) {
            ...
            var dotColor = diffDays <= 7 ? '#3E7A5D' : diffDays <= 30 ? '#B4863A' : '#A79C8B';
            ...
            if (isMe) html += ' <span style="font-size:.6rem; background:#E2EEE6; color:#2E5C46; padding:1px 7px; border-radius: var(--radius-sm); font-weight:600; vertical-align:middle;">Perangkat ini</span>';
            html += '</span>';
            html += '<span style="font-size:.65rem; color:#A79C8B;">' + d.count + ' aksi</span>';
            html += '</div>';
            html += '<div style="font-size:.65rem; color:#A79C8B; line-height:1.7;">';
