...
window.saveDeviceName = function() {
    ...
    if (st) { st.style.color = '#2F9E6E'; st.innerText = 'Tersimpan: ' + newId; }
...
window.loadConnectedDevices = async function() {
    ...
    try {
        ...
        if (!logs || !Array.isArray(logs) || logs.length === 0) {
            ...
            listEl.innerHTML = '<div style="font-size:.72rem; color:#98A1AD; text-align:center; padding:20px 0;">Belum ada log aktivitas di cloud.</div>';
            return;
        }
        ...
        devices.forEach(function(d) {
            ...
            var dotColor = diffDays <= 7 ? '#2F9E6E' : diffDays <= 30 ? '#D8A13B' : '#98A1AD';
            ...
            if (isMe) html += ' <span style="font-size:.6rem; background:#e3fcef; color:#1F7A54; padding:1px 7px; border-radius: var(--radius-sm); font-weight:600; vertical-align:middle;">Perangkat ini</span>';
            html += '</span>';
            html += '<span style="font-size:.65rem; color:#98A1AD;">' + d.count + ' aksi</span>';
            html += '</div>';
            html += '<div style="font-size:.65rem; color:#98A1AD; line-height:1.7;">';
